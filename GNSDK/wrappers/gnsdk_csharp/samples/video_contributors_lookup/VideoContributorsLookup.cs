...
namespace Sample
{
	public class VideoContributorsLookup
	{
		...
		static void
		FindContributor(GnDataObject workGDO, GnUser user)
		{
			int i = 0;
			GnVideo video = new GnVideo(user);

			GnResponseContributors responseContributor = video.FindContributors(workGDO);
		...
		static void
		DoContributorsLookup(GnUser user)
		{
			...
			using (GnStatusEventsDelegate videoEvents = new LookupStatusEvents())
			{
				...
				GnDataObject dataObject = GnDataObject.Deserialize(serializedGDO);
