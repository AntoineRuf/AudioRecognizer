...
/*
 *  Name: VideoSeasonsLookup.cs
 *  Description:
 *  This sample shows basic use of FindSeasons() call
 *
 *  Command-line Syntax:
...
namespace Sample
{
    public class VideoSeasonsLookup
    {
        /// <summary>
         ...
         */
        private static void
        DisplayEpisodes(GnVideoSeason season)
        {
            int count = 0;
            Console.WriteLine("\nNumber of Episodes: " + season.Works.count());

            GnVideoWorkEnumerable gnVideoWorkEnumerable = season.Works;
            foreach (GnVideoWork gnVideoWork in gnVideoWorkEnumerable)
            {
                /* Episode title */
                GnTitle episodeTitle = gnVideoWork.OfficialTitle;

                Console.WriteLine("\t" + ++count + " : " + episodeTitle.Display);
            }
        }
        ...
        DoSeasonsLookup(GnUser user)
        {
            ...
            using (LookupStatusEvents videoEvents = new LookupStatusEvents())
            {
                ...
                GnVideoSeason gnDataObject = new GnVideoSeason("238047322", "5C46058E04036C88490972A653C64A1D");

                Console.WriteLine("\nPerform the search...");

                GnResponseVideoSeasons gnResponseVideoSeasons = video.FindSeasons(gnDataObject);

                if (gnResponseVideoSeasons.Seasons.count() > 0)
                {
                    /****************************************************************
                    *  Needs decision (match resolution) check
                    *  We have at least one match - see if resolution is necessary.
                    ****************************************************************/
                    if (gnResponseVideoSeasons.NeedsDecision)
                    {
                        /**********************************************
                        * Resolve match here
                        **********************************************/
                    ...
                    GnVideoSeasonEnumerable gnVideoSeasonIterable = gnResponseVideoSeasons.Seasons;

                    foreach (GnVideoSeason season in gnVideoSeasonIterable)
                    {
                        GnVideoSeason videoSeason = season;
                        if (!season.IsFullResult)
                        {
                            GnResponseVideoSeasons gnResponseSeasons = video.FindSeasons(season);
                            videoSeason = gnResponseSeasons.Seasons.at(0).next();
                        }
                        GnTitle seasonTitle = videoSeason.OfficialTitle;
                        Console.WriteLine("\nTitle: " + seasonTitle.Display);

                        DisplayEpisodes(videoSeason);
                    }
                }
            }
        }
