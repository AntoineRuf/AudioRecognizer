 ...
 */
void
do_musicid_file(GnUser& user, GnMusicIdFileProcessType processType, GnMusicIdFileResponseType responseType)
{
...
display_result( GnResponseAlbums response )
{
	...
	int                      matchCounter = 0;
	album_iterable albums    = response.Albums();


	for (album_iterable::iterator it_album = albums.begin() ; it_album != response.Albums().end(); ++it_album)
	{
		std::cout << "\tMatch - Album Title:\t" << it_album->Title().Display() << std::endl;
		std::string url = it_album->Content(kContentTypeImageArtist).Asset(kImageSize300).Url();
		for ( track_iterator track_itr = it_album->TracksMatched().begin(); track_itr !=  it_album->TracksMatched().end(); ++track_itr)
		{
			std::cout << "\t\t Track Title:\t" << track_itr->Title().Display() << std::endl;

			for( string_iterator string_itr = track_itr->MatchedIdents().begin();string_itr != track_itr->MatchedIdents().end(); ++string_itr)
			{
				std::cout << "\tMatched " << ++matchCounter <<  "Identifiers :" << *string_itr << std::endl;
			}
		}
	}
...
main( int argc, char* argv[] )
{
	...
	try
	{
		...
		do_musicid_file(user, kQueryReturnSingle, kResponseAlbums);

		std::cout << "\n-------AlbumID with 'RETURN_ALL' option:-------"<< std::endl;
		do_musicid_file(user, kQueryReturnAll, kResponseAlbums);
	}
	catch (GnError& e)
	{
