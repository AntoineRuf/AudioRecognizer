...
display_match( GnDataMatch& dataMatch )
{
	...
	else if (dataMatch.IsContributor())
	{
		...
		GnName name = *(contrib.NamesOfficial().begin());
		printf( "%16s %s\n", "Name:", name.Display() );
	}
	else
	{
...
doMatchSelection(GnResponseDataMatches& match_response)
{
	/*
	...
	metadata::datamatch_iterator it_match = match_response.DataMatches().begin();
...
do_sample_text_query(gnsdk_cstr_t album_title, gnsdk_cstr_t track_title, gnsdk_cstr_t artist_name, GnUser& user)
{
	...
	/* Perform the query */
	GnResponseDataMatches match_response = musicid.FindMatches(album_title, track_title, artist_name, GNSDK_NULL, GNSDK_NULL);
	...
	else
	{
		...
		if (needs_decision)
		{
			choice_ordinal = doMatchSelection(match_response);
		...
		GnDataMatch match = *(match_response.DataMatches().at(choice_ordinal-1));
