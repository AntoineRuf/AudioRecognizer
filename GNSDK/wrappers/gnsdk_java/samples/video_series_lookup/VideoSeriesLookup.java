 ...
 */
/*
 * Name: video_series_lookup.VideoSeriesLookup
 * Description:
 * This sample shows basic use of the findSeries() API
 *
...
public class VideoSeriesLookup {
	...
	public static void main(String[] args) {
		...
		new VideoSeriesLookup().doSample( 
	...
	//
	void
	doSeriesLookup( GnUser user ) throws GnException {
		
		String tuiID  = "238073738";
		String tuiTag = "AE1B6F5143346B16AAD196EDC3093C6F";
		...
		GnVideo videoid = new GnVideo( user );

		// Create Video Series Object from TUI and TUI Tag
		GnVideoSeries dataObject = new GnVideoSeries( tuiID, tuiTag );

		System.out.println("\nPerform the search...");

		GnResponseVideoSeries videoSeriesResponse = videoid.findSeries(dataObject);

		GnVideoSeries series = videoSeriesResponse.series().getIterator().next();

		long count =  videoSeriesResponse.series().count();
		if (count == 0) {
			...
			boolean fullResult = series.isFullResult();

			// if we only have a partial result, we do a follow-up query to retrieve the full series
			if ( !fullResult ){
				
				// do followup query to get full object. Setting the partial video series as the query input.
				GnResponseVideoSeries followupResponse = videoid.findSeries(series);

				// Now our first video is the desired result with full data
				series = followupResponse.series().getIterator().next();

			}

			// Display Title and other Metadata
			System.out.println("\nTitle: " + series.officialTitle().display());		
			displaySeriesMetadata(series);
		}	
	}
	...
	//
	void
	displaySeriesMetadata( GnVideoSeries series ) {
		
		System.out.println( "Genre: " + series.genre(GnDataLevel.kDataLevel_1));

		System.out.println( "Production Type: " + series.videoProductionType());

		System.out.println( "Plot: " + series.plotSynopsis());

		System.out.println( "Reputation: " + series.reputation());
