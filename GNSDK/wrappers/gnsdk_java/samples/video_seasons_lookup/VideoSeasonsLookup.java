 ...
 */
/*
 * Name: video_seasons_lookup.VideoSeasonsLookup
 * Description:
 * This sample shows basic use of the findSeasons() API
 *
...
public class VideoSeasonsLookup {
	...
	public static void main(String[] args) {
		...
		new VideoSeasonsLookup().doSample( 
	...
	//
	void
	doSeasonsLookup( GnUser user ) throws GnException {
		
		String tuiID  = "238047322";
		String tuiTag = "5C46058E04036C88490972A653C64A1D";
		...
		GnVideoSeason seasonObject = new GnVideoSeason(tuiID, tuiTag);

		System.out.println("\nPerforming the search...");

		GnResponseVideoSeasons videoSeasonsResponse = videoid.findSeasons(seasonObject);

		long count =  videoSeasonsResponse.seasons().count();
		if (count == 0) {
			...
			GnVideoSeason season = videoSeasonsResponse.seasons().getIterator().next();
			boolean fullResult = season.isFullResult();
			...
			if ( !fullResult ) {
				...
				GnResponseVideoSeasons followupResponse = videoid.findSeasons(season);

				// Now our first video is the desired result with full data
				season = followupResponse.seasons().getIterator().next();

			}

			// Display Title and other Metadata
			System.out.println("\nTitle: " + season.officialTitle().display());	
	...
	//
	void
	displayEpisodes( GnVideoSeason season ) throws GnException
	{
		System.out.println("\nNumber of Episodes: " + season.works().count());

		GnVideoWorkIterator episodeItr = season.works().getIterator();

		int i = 1;
		while ( episodeItr.hasNext() ){
			GnVideoWork episode = episodeItr.next();	
			System.out.println("\t" + i++ + " : " + episode.officialTitle().display());
		}
	}
