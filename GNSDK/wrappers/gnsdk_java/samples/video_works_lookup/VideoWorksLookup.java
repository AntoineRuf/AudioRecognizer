 ...
 */
/*
 * Name: video_works_lookup.VideoWorksLookup
 * Description:
 * This sample shows basic use of the findWorks() API
 *
...
public class VideoWorksLookup {
	...
	doVideoWorkLookup( GnUser user ) throws GnException {
		...
		GnResponseVideoWork videoWorkResponse = videoid.findWorks(searchText, GnVideoSearchField.kSearchFieldContributorName, GnVideoSearchType.kSearchTypeDefault);

		long count =  videoWorkResponse.works().count();
		if (count == 0) {
			...
			System.out.println("\nAV Works for " + searchText + ":");

			GnVideoWorkIterator workIterator = videoWorkResponse.works().getIterator();			

			int i = 1;
			while ( workIterator.hasNext() ){
				
				GnVideoWork work = workIterator.next();
				
				// Work title
				System.out.println("\t" + i++ + ": " + work.officialTitle().display());
