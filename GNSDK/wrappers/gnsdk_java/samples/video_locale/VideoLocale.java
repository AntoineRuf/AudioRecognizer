...
public class VideoLocale {
	...
	//
	void
	doWorkSearch(GnUser user) throws GnException {
	...
	//	
	void
	displayWorkMetadata( GnVideoWork work ) {
		
		System.out.println("\nTitle: " + work.officialTitle().display());	

		// Origin info (origins support 4 levels) 
		if ( work.origin(GnDataLevel.kDataLevel_1) != null ) // if an origin exists for one level, it exists for all
		{
			System.out.println("Origin1: " + work.origin(GnDataLevel.kDataLevel_1));
			System.out.println("Origin2: " + work.origin(GnDataLevel.kDataLevel_2));
			System.out.println("Origin3: " + work.origin(GnDataLevel.kDataLevel_3));
			System.out.println("Origin4: " + work.origin(GnDataLevel.kDataLevel_4));
		}

		// Genre info (genre supports 3 levels)
		if ( work.genre(GnDataLevel.kDataLevel_1) != null ) // if a genre exists for one level, it exists for all
		{
			System.out.println("Genre1: " + work.genre(GnDataLevel.kDataLevel_1));
			System.out.println("Genre2: " + work.genre(GnDataLevel.kDataLevel_2));
			System.out.println("Genre3: " + work.genre(GnDataLevel.kDataLevel_3));
		}

		// Video mood
		if ( work.videoMood() != null ) {
			System.out.println("Mood: " + work.videoMood());
		}

		// Primary rating
		GnRating rating = work.rating();
		if (rating.rating() != null)
			System.out.print("Rating: " + rating.rating());
		if (rating.ratingType() != null)
			System.out.print(" [" + rating.ratingType() + "]");	
		if (rating.ratingDesc() != null)
			System.out.print(" [%s]" + rating.ratingDesc());

		if (rating.ratingReason() != null)
			System.out.print(" Reason: " + rating.ratingReason());
		System.out.println("\n");

		// Plot synopsis
		if ( work.plotSynopsis() != null ) {
			System.out.println("Plot synopsis: " + work.plotSynopsis());
		}
	}
	...
	//
	void
	displayCredit( GnVideoWork work ) throws GnException {
		
		GnVideoCredit credit = work.videoCredits().getIterator().next();
		
		// Display name 	
		System.out.println( "\n\tChatacter Name: " + credit.characterName());

		// Genres 
		if (credit.genre(GnDataLevel.kDataLevel_1) != null) // if a genre exists for one level, it exists for all
		{
			System.out.println("\tGenre1: " + credit.genre(GnDataLevel.kDataLevel_1));
			System.out.println("\tGenre2: " + credit.genre(GnDataLevel.kDataLevel_2));
			System.out.println("\tGenre3: " + credit.genre(GnDataLevel.kDataLevel_3));
		}
	}	
