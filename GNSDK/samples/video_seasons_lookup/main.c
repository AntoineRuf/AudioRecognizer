...
/*
 *  Name: video_seasons_lookup/main.c
 *  Description:
 *  This sample shows basic use of the gnsdk_video_query_find_seasons() call
 *
 *  Command-line Syntax:
...
main(int argc, char* argv[])
{
	...
	if ((argc == 4) || (argc == 5))
	{
		...
		if (0 == rc)
		{
			...
			rc = _do_seasons_lookup(user_handle);
...
******************************************************************/
void
_display_episodes(gnsdk_gdo_handle_t response_gdo)
{
	gnsdk_error_t      error            = GNSDK_SUCCESS;
	gnsdk_gdo_handle_t episode_name_gdo = GNSDK_NULL;
	gnsdk_gdo_handle_t episode_gdo      = GNSDK_NULL;
	gnsdk_cstr_t       value            = GNSDK_NULL;
	gnsdk_uint32_t     i                = 1;      /* Child GDOs always start at 1 */
	...
	else
	{
		...
		for (i = 1; i <= count; i++)
		{
			error = gnsdk_manager_gdo_child_get(response_gdo, GNSDK_GDO_CHILD_VIDEO_WORK, i, &episode_gdo);
			if (GNSDK_SUCCESS != error)
			{
				_display_last_error(__LINE__);
			}
			...
			{
				error = gnsdk_manager_gdo_child_get(episode_gdo, GNSDK_GDO_CHILD_TITLE_OFFICIAL, 1, &episode_name_gdo);
				if (GNSDK_SUCCESS != error)
				{
					_display_last_error(__LINE__);
				}
				...
				{
					error = gnsdk_manager_gdo_value_get(episode_name_gdo, GNSDK_GDO_VALUE_DISPLAY, 1, &value);
					if (GNSDK_SUCCESS != error)
					{
						_display_last_error(__LINE__);
					}
			...
			gnsdk_manager_gdo_release(episode_gdo);
			gnsdk_manager_gdo_release(episode_name_gdo);
...
******************************************************************/
static int
_do_seasons_lookup(gnsdk_user_handle_t user_handle)
{
	gnsdk_error_t              error             = GNSDK_SUCCESS;
	...
	/*********************************
	 * Get Simpson's First Season GDO
	 **********************************/
	...
	else
	{
		...
		error = gnsdk_manager_gdo_create_from_id("238047322",
												 "5C46058E04036C88490972A653C64A1D",
		...
		else
		{
			...
			{
				printf("\nPerform the search...\n");
				error = gnsdk_video_query_find_seasons(query_handle, &response_gdo);
				if (GNSDK_SUCCESS != error)
				{
					_display_last_error(__LINE__);
				}
	...
	if (GNSDK_SUCCESS == error)
	{
		error = gnsdk_manager_gdo_child_count(response_gdo, GNSDK_GDO_CHILD_VIDEO_SEASON, &count);
		if (GNSDK_SUCCESS != error)
		{
			_display_last_error(__LINE__);
		}
	...
	if (GNSDK_SUCCESS == error)
	{
		error = gnsdk_manager_gdo_child_get(response_gdo, GNSDK_GDO_CHILD_VIDEO_SEASON, 1, &work_gdo );
		if (GNSDK_SUCCESS != error)
		{
			_display_last_error(__LINE__);
		}
				...
				if (0 == strcmp(is_full, GNSDK_VALUE_FALSE))
				{
					...
					else
					{
						...
						else
						{
							...
							error = gnsdk_video_query_find_seasons(query_handle, &full_response_gdo);
							if (GNSDK_SUCCESS != error)
							{
								_display_last_error(__LINE__);
							}
							else
							{
								...
								error = gnsdk_manager_gdo_child_get(full_response_gdo, GNSDK_GDO_CHILD_VIDEO_SEASON, 1, &work_gdo);

								/* Release the followup query's response object */
								gnsdk_manager_gdo_release(full_response_gdo);

							}     /* else find_seasons ok*/
						}     /* else set_gdo ok */
					}      /* if query_create ok */
				}     /* end if not full */
	...
	if (GNSDK_SUCCESS == error)
	{
		...
		else
		{
			...
			{
				printf("\nTitle: %s\n", value);
				_display_episodes(work_gdo);
			}
			else _display_last_error(__LINE__);

		}     /* end else get title GDO ok */
	}
	...
	*********************************/
	...
	return (GNSDK_SUCCESS != error) ? -1 : 0;
}  /* _do_seasons_lookup() */
