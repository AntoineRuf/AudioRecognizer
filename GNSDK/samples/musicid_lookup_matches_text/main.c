	...
	if (argc == 5)
	{
		...
		if (use_local != -1)
		{
			...
			if (0 == rc)
			{
				...
				_do_sample_text_queries(user_handle);
 ...
 *
 *    Perform a text query and display results.
 *
 ***************************************************************************/
static void
_do_sample_text_query(
	...
	)
{
	...
	_do_sample_text_query( "看我72变", GNSDK_NULL, GNSDK_NULL, user_handle );
	
	/* Perform a sample text query with only the Artist name as an input. */
	_do_sample_text_query( GNSDK_NULL, GNSDK_NULL, "Philip Glass", user_handle );
	_do_sample_text_query( GNSDK_NULL, GNSDK_NULL, "Bob Marley", user_handle );
	
	/* Perform a sample text query with Track Title and Artist name. */
	_do_sample_text_query( GNSDK_NULL, "Purple Stain", "Red Hot Chili Peppers", user_handle );
	_do_sample_text_query( GNSDK_NULL, "Eyeless", "Slipknot", user_handle );
	 ...
	 */
	_do_sample_text_query( "民谣风情", GNSDK_NULL, GNSDK_NULL, user_handle );		/*	simplified input	*/
	_do_sample_text_query( "民謠風情", GNSDK_NULL, GNSDK_NULL, user_handle );		/*	traditional input	*/
