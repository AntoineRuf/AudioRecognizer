	...
	if (argc == 5)
	{
		...
		if (use_local != -1)
		{
			...
			if (0 == rc)
			{
				...
				query_album_lookup(user_handle);
...
*	DISPLAY_ALBUM_GDO
*
*	Display basic album metadata.
*
******************************************************************/
void
display_album_gdo(gnsdk_gdo_handle_t album_gdo)
{
	gnsdk_error_t      error     = GNSDK_SUCCESS;
 ...
 */
void
display_for_resolve(gnsdk_gdo_handle_t response_gdo)
{
	gnsdk_error_t      error         = GNSDK_SUCCESS;
 ...
 */
gnsdk_uint32_t
do_match_selection(gnsdk_gdo_handle_t response_gdo)
{
	/*
	 ...
	 */
	display_for_resolve(response_gdo);
 ...
 */
void
query_album_lookup(gnsdk_user_handle_t user_handle)
{
	gnsdk_error_t                error                 = GNSDK_SUCCESS;
	...
	if (GNSDK_SUCCESS == error)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					choice_ordinal = do_match_selection(response_gdo);
				}
				else
				{
